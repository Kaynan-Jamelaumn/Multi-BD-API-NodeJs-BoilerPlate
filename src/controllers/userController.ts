...
class UserController {
  ...
  private getProfilePicturePaths(req: RequestWithFile): { profilePicture: string | null, profilePictureTruePath: string | null } {
  ...
  public login = async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      await dbManager.update(user.id, { lastLogin: new Date() });
  ...
  public searchUsers = async (req: Request, res: Response): Promise<any> => {
    try {
      ...
      const { count, rows } = await dbManager.findAndCount(query, {
        ...
        sort: [[sortBy as string, (order as string).toUpperCase()]], // Apply sorting
  ...
  private failedValidationAndDeletePhoto(
    req: Request,
    res: Response,
    validationError: { valid: boolean; status: number; error: string | null } | null, 
    ...
    if (validationError && !validationError.valid) {
      ...
      res.status(validationError.status).json({ error: validationError.error || null }); 
      return true;
    }
    return false;
  }
  ...
  private async checkFieldAvailability(
    ...
    try {
      ...
      if (!existingUser) {
        ...
        if (user) {
          ...
          this.failedValidationAndDeletePhoto(
            req,
            res,
            { valid: false, error: `${fieldName} is already in use`, status: 400 },
            pathToDelete
          );
          return false;
        }
      ...
      const whereClause = {
        ...
        id: { [Op.ne]: existingUser.id } // Exclude the current user
      };
