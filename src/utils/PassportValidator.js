class PassportValidator {
    static validatePassport(passportNumber, countryCode) {
        ...
        if (!(countryCode in passportPatterns)) {
            return {
                valid: false,
        ...
        const countriesWithChecksum = new Set([
            'FR', 'DE', 'RU', 'UK', 'US', 'NL', 'SE', 'CH', 'IE', 'IT', 'PL', 'GR', 'NO', 'FI'
        ]);

        if (countriesWithChecksum.has(countryCode)) {
            if (['NO', 'FI'].includes(countryCode)) {
                if (!this.validateMod11Checksum(passportNumber)) {
                    return {
                        valid: false,
                ...
                if (!this.validateICAOChecksum(passportNumber)) {
                    return {
                        valid: false,
    ...
    /*
      ...
      A=10, B=11, ..., Z=35.
     */
    static validateICAOChecksum(number) {
        ...
        let expectedChecksum = sum % 10;

        // Parse the actual checksum digit (last character of the number)
        let actualChecksum = parseInt(number[number.length - 1], 10);
     ...
     */
    static validateMod11Checksum(number) {
        let sum = 0;
        ...
        for (let i = 0; i < number.length - 1; i++) {
            let value = parseInt(number[i], 10); // Convert the character to an integer
